...
export default function LeaderboardPage() {
  ...
  const fetchLeaderboard = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/community/users?sort=reputation&limit=20`)
  ...
  const achievements = [
    { title: 'Most Helpful Answers', winner: topUsers[0]?.name || 'Loading...', count: 156, icon: '🤝' },
    { title: 'Best Questions', winner: topUsers[1]?.name || 'Loading...', count: 89, icon: '❓' },
    { title: 'Most Upvoted Posts', winner: topUsers[2]?.name || 'Loading...', count: 1247, icon: '👍' },
    { title: 'Community Champion', winner: topUsers[3]?.name || 'Loading...', count: 45, icon: '🏆' }
  ]
