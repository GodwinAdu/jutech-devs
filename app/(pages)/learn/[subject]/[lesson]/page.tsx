...
import { allLessons, subjectLessons, courseStructure } from "@/lib/lessons"
...
export default function LessonPage() {
  ...
  const lessonData = allLessons[lessonId as keyof typeof allLessons]
  if (!lessonData) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">Lesson Not Found</h1>
          <p className="text-muted-foreground mb-4">The lesson "{lessonId}" does not exist.</p>
  ...
  const allSubjectLessons = courseStructure.frontend.modules.flatMap(module => 
    module.lessons.map(id => ({
      id,
      title: allLessons[id as keyof typeof allLessons]?.title || id,
      active: id === lessonId,
      subject: module.id
    }))
  )
      ...
      <div className="flex pt-16">
        ...
        <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-card border-r transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static`}>
          <div className="p-4 space-y-4 h-full overflow-y-auto">
            <div>
              <h3 className="font-semibold text-sm text-muted-foreground mb-2">All Lessons</h3>
              <div className="space-y-1">
                {allSubjectLessons.map((lesson) => (
                  <Link
                    key={lesson.id}
