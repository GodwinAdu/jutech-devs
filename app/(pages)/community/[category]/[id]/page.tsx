  ...
  const fetchPost = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setDownvoteCount(data.post.downvoteCount || 0)
      } else {
            ...
            <div className="flex items-start gap-8 mb-6">
              <div className="flex flex-col items-center gap-3 bg-muted/30 rounded-xl p-4">
                <Button 
                  size="sm" 
                  variant="ghost" 
                  className={`p-3 rounded-lg transition-all ${userVote === 'upvote' ? 'bg-green-500/20 text-green-500 hover:bg-green-500/30' : 'hover:bg-green-500/10 hover:text-green-500'}`}
                ...
                <div className="text-center">
                  <div className="text-lg font-bold text-green-600">{upvoteCount}</div>
                  <div className="text-lg font-bold text-red-600">{downvoteCount}</div>
                </div>
                <Button 
                  size="sm" 
                  variant="ghost" 
                  className={`p-3 rounded-lg transition-all ${userVote === 'downvote' ? 'bg-red-500/20 text-red-500 hover:bg-red-500/30' : 'hover:bg-red-500/10 hover:text-red-500'}`}
